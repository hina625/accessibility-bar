...
import { getTranslations } from '@/contexts/accessibility/translations';
...
const ACCESSIBILITY_PROFILES = [
  ...
  { id: 'dyslexia', name: 'Dyslexia Friendly' },
  ...
  { id: 'elderly', name: <span>Seniors / <br />Silver Surfers</span> },
...
export default function AccessibilityBar() {
  ...
  const [hoveredCategory, setHoveredCategory] = useState<{ id: string, name: string, rect: DOMRect } | null>(null);
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      ...
      const isInside = (ref: React.RefObject<any>) => {
        return ref.current && path.includes(ref.current);
      };

      if (isInside(panelRef)) return;

      if (showSettingsDropdown && isInside(settingsModalRef)) return;

      if (showMoreMenu && isInside(moreMenuRef)) return;

      if (selectedCategory && isInside(categoryPanelRef)) return;

      if (showSettingsDropdown || showMoreMenu || selectedCategory) {

        if (selectedCategory === 'contact' || selectedCategory === 'info') return;
  ...
  }, [showSettingsDropdown, showMoreMenu, selectedCategory]);
  ...
  useEffect(() => {
    ...
    const checkEmbed = () => {
      const embedHost = document.querySelector('.a11y-embed-host') || document.getElementById('a11y-embed-host-react');
      setIsEmbed(!!embedHost);
    };
    checkEmbed();
  }, []);
  ...
  useEffect(() => {
    if (selectedCategory && !isVertical && isOpen) {
      const updateOffset = () => {
        ...
        if (!selectedButton && (selectedCategory === 'feedback' || selectedCategory === 'info')) {
          selectedButton = document.querySelector('[data-more-btn]') as HTMLElement;
        }
  ...
  const categories = [
    ...
    { id: 'letterSpacing', name: `Letter Spacing`, icon: letterSpacingIcon, colorClass: 'from-indigo-500 to-indigo-600', indicatorClass: 'bg-indigo-500' },
    {
    ...
    { id: 'az', name: `A-Z`, icon: azIcon, colorClass: 'from-gray-500 to-gray-600', indicatorClass: 'bg-gray-500' },
    ...
    { id: 'contact', name: t.common?.contact || `Contact Us`, icon: contactUsIcon, colorClass: 'from-gray-500 to-gray-600', indicatorClass: 'bg-gray-500' },
  ...
  const renderCategoryContent = () => {
    switch (selectedCategory) {
      ...
      case 'position':
        return (
          <div className="space-y-4">
            <PositionControls onPositionSelected={() => setSelectedCategory(null)} />
      ...
      case 'ai':
        ...
        return (
          <div className="space-y-4">
            <PageSummaryControl />
            <div className="border-b-4 -mx-6 my-4" style={{ borderColor: `${currentTheme.border}`, borderBottomWidth: '3px', boxShadow: 'none' }} />
            <h3 className="text-[15px] font-normal tracking-wider mb-2 px-1" style={{ color: currentTheme.text }}>
              AI Powered Tools
            </h3>
              ...
              {aiShortcuts.filter(s => s.id !== 'page_summary').map((shortcut) => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="text-[17px] font-normal leading-tight whitespace-nowrap" style={{ color: currentTheme.text }}>
                      {shortcut.name}
                    </div>
                  </div>
                  <svg
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              {Object.entries(t.info || {}).map(([key, data]: [string, any]) => {
                ...
                if (key === 'reset' || key === 'info' || key === 'feedback' || key === 'contact') return null;
      ...
      case 'contact':
        return (
          <div className="space-y-6">
            <ContactPage onClose={() => setSelectedCategory(null)} />
          </div>
        );
      default:
        return null;
  ...
  return (
    <>
        ...
        <>
          ...
          <button
            ...
            onClick={(e) => {
              ...
              // setTextToSpeech?.(true); // Removed to respect user preference
            ...
            className={`accessibility-bar a11y-embed-host fixed z-[2147483640] flex h-20 w-20 items-center justify-center rounded-full text-white transition-all duration-300 ease-out hover:scale-110 focus:outline-none focus:ring-4 focus:ring-offset-2 overflow-hidden cursor-pointer ${getButtonPositionClasses()}`}
            style={{
      ...
      {/* Global More menu dropdown (positioned outside bar like panels) */}
      {
        showMoreMenu && moreMenuPosition && (
          <div
            ...
            style={{
              top: (!isVertical && panelPosition === 'bottom') ? 'auto' : moreMenuPosition.y,
              bottom: (!isVertical && panelPosition === 'bottom') ? `calc(100vh - ${moreMenuPosition.y}px)` : 'auto',
      ...
      {isOpen && (
        <>
          ...
          <div
            ...
            onMouseDown={(e) => e.stopPropagation()}
            onMouseUp={(e) => e.stopPropagation()}
            className={`accessibility-bar a11y-embed-host fixed z-[2147483640] ${panelBorderStyle} transition-all duration-300 ease-out overflow-visible pointer-events-auto ${getPanelPositionClasses()} ${isVertical ? 'top-0 bottom-0' : 'left-0 right-0'}`}
            style={{
              width: isVertical
                ? (selectedCategory
              ...
              height: isVertical
                ...
                : ((panelPosition === 'top' || panelPosition === 'bottom') &&
                  (sidebarIconSize === 1.3 || sidebarIconSize === 1.5))
                  ? `${90 * sidebarIconSize}px`
                  : `${80 * sidebarIconSize}px`,
          ...
          >
              ...
              style={{
                ...(isVertical
                  ...
                  : {
                    ...
                    // Desktop: keep the three buttons (A–Z, +, Sidebar Position) tightly grouped
                    // in the center with auto width; Mobile: still stretch nearly full-width.
            ...
            >
              <div
                className={`w-full h-full flex ${isVertical ? 'flex-col' : 'flex-row overflow-x-auto custom-scrollbar'} items-center ${isVertical ? 'justify-between' : 'justify-center'} pointer-events-auto`}
                style={{ gap: isVertical ? '4px' : '8px' }}
              >
                {/* Left: A-Z List (text-sized button) */}
                <button
                  onClick={async (e) => {
                    e.stopPropagation();
                    const target = e.currentTarget; // cache before awaiting (React pools events)
                    if (audioPingEnabled) playAudioPing();
                    if (textToSpeech) {
                      await speak('A to Z List', ttsVoiceGender);
                    }
                  ...
                  onMouseEnter={(e) => {
                    ...
                    setHoveredCategory({ id: 'az', name: 'A to Z List', rect });
                    if (textToSpeech && !justOpenedRef.current) {
                      speak('A to Z List', ttsVoiceGender);
                    }
                  }}
                  onMouseLeave={() => setHoveredCategory(null)}
                  className={`flex-none flex items-center justify-center pointer-events-auto transition-all duration-300 ${isVertical ? 'w-[32px]' : 'h-[32px]'} ${isVertical ? 'px-1' : 'px-3'} border-0 focus:outline-none ease-out`}
                  style={{
                    borderRadius: '0px',
                    background: `linear-gradient(135deg, ${currentTheme.background}, ${currentTheme.hover})`,
                    boxShadow: `0 3px 8px ${currentTheme.border}33`,
                    border: `1px solid ${currentTheme.border}4D`,
                    backdropFilter: 'blur(10px)',
                    WebkitBackdropFilter: 'blur(10px)',
                    filter: 'brightness(1)',
                    minWidth: isVertical ? '32px' : '60px',
                    minHeight: isVertical ? '60px' : '32px'
                  }}
                  title="A-Z List"
                >
                  <span
                ...
                <button
                  ...
                  onClick={(e) => {
                    ...
                    if (newShowSettings) {
                      ...
                      setShowMoreMenu(false); // Close More Menu when opening settings
                    }
                  }}
                  onMouseEnter={(e) => {
                    const rect = e.currentTarget.getBoundingClientRect();
                    setHoveredCategory({ id: 'customise', name: 'Customise', rect });
                    if (textToSpeech && !justOpenedRef.current) {
                ...
                {/* Right: Sidebar Position (icon button, slightly bigger than icon) */}
                <button
                  onClick={(e) => {
                    e.stopPropagation();
                    if (audioPingEnabled) playAudioPing();
                  ...
                  onMouseEnter={(e) => {
                    const rect = e.currentTarget.getBoundingClientRect();
                    setHoveredCategory({ id: 'position', name: 'Sidebar Position', rect });
                    if (textToSpeech && !justOpenedRef.current) {
                ...
                return canRenderPortal ? createPortal(
                  <>
                    ...
                    >
                      ...
                      <div
                        className="flex flex-col overflow-hidden rounded-none flex-1 flex-shrink-0 relative"
                        style={{
                          backgroundColor: currentTheme.background,
                          ...
                          borderStyle: 'solid',
                          color: currentTheme.text,
                          height: isVertical ? '100vh' : `calc(100vh - ${100 * sidebarIconSize}px)`,
                          maxHeight: isVertical ? '100vh' : `calc(100vh - ${100 * sidebarIconSize}px)`
                        }}
                      >
                        {/* Compact Header */}
                        <div className="px-3 sm:px-4 md:px-5 py-2 border-b flex items-center justify-center relative flex-shrink-0" style={{ borderColor: modalBorderColor }}>
                          ...
                          <button
                            ...
                            onMouseEnter={(e) => {
                              const rect = e.currentTarget.getBoundingClientRect();
                              setHoveredCategory({ id: 'close', name: 'Close', rect });
                            }}
                            onMouseLeave={() => setHoveredCategory(null)}
                          ...
                          >
                            ...
                            {!isMobile && <span className="text-sm font-normal" style={{ color: currentTheme.text }}>Exit</span>}
                          </button>
                        </div>
                        ...
                        <div
                          ...
                          className="grid gap-0 overflow-y-auto overflow-x-hidden icons-scroll-hidden flex-1 min-h-0"
                          style={{
                            ...
                            <div className="grid grid-cols-2 gap-1.5">
                              {[
                                ...
                                >
                                  ...
                                  {/* Label */}
                                  <span
                                    className={`text-[13px] font-normal tracking-wider text-center leading-tight`}
                                    style={{ color: currentTheme.text }}
                                  >
                                    {opt.name}
                                  </span>
                                  ...
                                  .map(([key, theme]) => {
                                    ...
                                    return (
                                      ...
                                      >
                                        ...
                                        <span className="text-[10px] font-normal tracking-tight text-center leading-tight" style={{ color: currentTheme.text, opacity: isSelected ? 1 : 0.9 }}>
                                          {label}
                                        </span>
                                      </button>
                                    );
                                  ...
                                  .map(([key, theme]) => {
                                    ...
                                    return (
                                      ...
                                      >
                                        ...
                                        <span className="text-[10px] font-bold tracking-tight text-center leading-tight whitespace-nowrap" style={{ color: currentTheme.text, opacity: isSelected ? 1 : 0.9 }}>
                                          {label}
                                        </span>
                                      </button>
                                    );
              ...
              {showSettingsDropdown && !isMobile && (
                <div
                  ref={settingsModalRef}
                  className="absolute z-[2147483651] rounded-none shadow-2xl border-4 flex flex-col custom-scrollbar"
                  onMouseEnter={() => {
                    if (textToSpeech) {
                      speak('Customise', ttsVoiceGender);
                    }
            ...
            <div className={`flex h-full w-full ${isVertical
              ...
              >
                ...
                <button
                  ...
                  onClick={async (e) => {
                    ...
                    // Speak "Exit Toolbar" when close button is clicked (only if TTS is enabled)
                    if (textToSpeech) {
                      await speak('Exit Toolbar', ttsVoiceGender);
                    }
                  ...
                  onMouseEnter={(e) => {
                    const rect = e.currentTarget.getBoundingClientRect();
                    setHoveredCategory({ id: 'close', name: 'Exit Toolbar', rect });
                    if (textToSpeech && !justOpenedRef.current) {
                      speak('Exit Toolbar', ttsVoiceGender);
                    }
                  }}
                  onMouseLeave={() => setHoveredCategory(null)}
                ...
                {isVertical && (
                  <button
                    ...
                    onClick={(e) => {
                      ...
                      let x = (panelRect ? panelRect.right : rect.right) + 0;
                      if (panelPosition === 'right') {
                        x = (panelRect ? panelRect.left : rect.left) - menuWidth - 0;
                      }

                      // Clamp x to stay on screen
                      x = Math.max(8, Math.min(viewportWidth - menuWidth - 8, x));

                      // Move moves up (approx height of menu is ~200px)
                      // "thoda upr kro" -> move higher (subtract more)
                      let y = rect.bottom - 260;
                    ...
                    onMouseEnter={(e) => {
                      const rect = e.currentTarget.getBoundingClientRect();
                      setHoveredCategory({ id: 'more', name: 'More Options', rect });
                      if (textToSpeech && !justOpenedRef.current) {
                        speak('More options', ttsVoiceGender);
                      }
                    }}
                    onMouseLeave={() => setHoveredCategory(null)}
                    className="p-2 mt-1 rounded-xl transition-all duration-300 pointer-events-auto hover:bg-black/10 active:scale-95 flex items-center justify-center"
                    style={{
                      width: '60px',
                      height: '60px',
                  ...
                  >
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width={30 * sidebarIconSize}
                      height={30 * sidebarIconSize}
                      viewBox="0 0 24 24"
                      fill={currentTheme.text === '#FFFFFF' ? '#FFFFFF' : '#000000'}
                    >
                ...
                >
                  ...
                  {(() => {
                    ...
                    const paginatedCategories = categories.filter(c => !['reset', 'az', 'position', 'feedback', 'contact', 'info'].includes(c.id));
                    ...
                    return (
                      <>
                              ...
                              {resetCategory && (
                                <div key="reset-constant" className="relative group/category">
                                  <button
                                    ...
                                    onMouseEnter={(e) => {
                                      const rect = e.currentTarget.getBoundingClientRect();
                                      setHoveredCategory({ id: 'reset', name: resetCategory.name, rect });
                                      if (textToSpeech && !justOpenedRef.current) speak(resetCategory.name.replace(/\n/g, ' '), ttsVoiceGender);
                                    }}
                                    onMouseLeave={() => setHoveredCategory(null)}
                                  ...
                                  >
                                    <Image
                                      ...
                                      alt=""
                                      width={(isVertical ? 50 : (!isVertical && (sidebarIconSize === 1.3 || sidebarIconSize === 1.5)) ? 40 : 44) * sidebarIconSize}
                                      height={(isVertical ? 50 : (!isVertical && (sidebarIconSize === 1.3 || sidebarIconSize === 1.5)) ? 40 : 44) * sidebarIconSize}
                                      style={{
                          ...
                          <div key={category.id} className="relative group/category">
                            <button
                              ...
                              onMouseEnter={(e) => {
                                ...
                                if (category.id === 'az') name = 'A to Z List';
                                if (category.id === 'position') name = 'Sidebar Position';

                                setHoveredCategory({ id: category.id, name, rect });
                                if (textToSpeech && !justOpenedRef.current) {
                                ...
                                ...(category.id === 'reset'
                                  ? {
                                    ...
                                    width: (isVertical
                                      ...
                                      : (sidebarIconSize === 1.3 || sidebarIconSize === 1.5)
                                        ? 80
                                        : 58) * sidebarIconSize + 'px',
                                    height: (isVertical ? 80 : (sidebarIconSize === 1.3 || sidebarIconSize === 1.5) ? 80 : 58) * sidebarIconSize + 'px'
                                  }
                                  : category.id === 'az'
                                    ? {
                                      background: '#FFFFFF',
                                      ...
                                      border: '2px solid rgba(0,0,0,0.2)',
                                      boxShadow: '0 2px 8px rgba(0,0,0,0.1)',
                                      width: (isVertical ? 48 : (!isVertical && (sidebarIconSize === 1.3 || sidebarIconSize === 1.5)) ? 58 : 50) * sidebarIconSize + 'px',
                                      height: (isVertical ? 35 : (!isVertical && (sidebarIconSize === 1.3 || sidebarIconSize === 1.5)) ? 45 : 38) * sidebarIconSize + 'px'
                                    }
                                    : selectedCategory === category.id
                                      ? {
                                        ...
                                        boxShadow: 'none',
                                        width: (isVertical ? (category.id === 'position' ? 64 : 78) : (!isVertical && (sidebarIconSize === 1.3 || sidebarIconSize === 1.5)) ? (category.id === 'position' ? 64 : 80) : (category.id === 'position' ? 48 : 58)) * sidebarIconSize + 'px',
                                        height: (!isVertical && (sidebarIconSize === 1.3 || sidebarIconSize === 1.5))
                                          ? ((category.id === 'position' ? 45 : 66) * sidebarIconSize + (category.id === 'position' ? 0 : 10)) + 'px'
                                          : (isVertical ? (category.id === 'position' ? 35 : 80) : (category.id === 'position' ? 38 : 58)) * sidebarIconSize + 'px'
                                      }
                                      : {
                                        background: 'linear-gradient(135deg, rgba(255, 255, 255, 0.95), rgba(255, 255, 255, 0.85))',
                                        backdropFilter: 'blur(5px) saturate(180%)',
                            ...
                            >
                              {category.id === 'az' ? (
                                ...
                                >
                                  <span style={{ fontSize: `${(isVertical ? 18 : (!isVertical && (sidebarIconSize === 1.3 || sidebarIconSize === 1.5)) ? 18 : 22) * sidebarIconSize}px`, display: 'block', lineHeight: '1', fontWeight: 'normal', whiteSpace: 'nowrap' }}>A  to  Z</span>
                                </div>
                              ) : (
                                <div className="flex items-center justify-center" style={{ paddingTop: (!isVertical && (sidebarIconSize === 1.3 || sidebarIconSize === 1.5)) ? '24px' : (isVertical ? '4px' : '2px') }}>
                                  <Image
                                    src={
                                    ...
                                    alt=""
                                    width={(isVertical ? (category.id === 'reset' ? 50 : (category.id === 'images' ? 38 : (category.id === 'info' ? 36 : 30))) : (!isVertical && (sidebarIconSize === 1.3 || sidebarIconSize === 1.5)) ? (category.id === 'reset' ? 34 : (category.id === 'images' ? 30 : (category.id === 'info' ? 30 : 27))) : (category.id === 'reset' ? 40 : (category.id === 'images' ? 36 : (category.id === 'info' ? 36 : 30)))) * sidebarIconSize}
                                    height={(isVertical ? (category.id === 'reset' ? 50 : (category.id === 'images' ? 38 : (category.id === 'info' ? 36 : 30))) : (!isVertical && (sidebarIconSize === 1.3 || sidebarIconSize === 1.5)) ? (category.id === 'reset' ? 34 : (category.id === 'images' ? 30 : (category.id === 'info' ? 30 : 27))) : (category.id === 'reset' ? 40 : (category.id === 'images' ? 36 : (category.id === 'info' ? 36 : 30)))) * sidebarIconSize}
                                    style={{
                        ...
                        {/* Three-dots More menu at the end of icon bar */}
                        {!isVertical && (
                          <div className="relative group/category ml-auto pl-1">
                            {/* Visually just three dots, no tile background */}
                            <button
                              type="button"
                              onClick={(e) => {
                                e.stopPropagation();
                                ...
                                if (!isVertical && panelPosition === 'top' && panelRect) {
                                  dropdownY = panelRect.bottom;
                                } else if (!isVertical && panelPosition === 'bottom') {

                                  const baseTop = panelRect ? panelRect.top : rect.top;
                                  dropdownY = baseTop + 10;
                                }
                                ...
                                const clampedLeft =
                                  viewportWidth > 0
                                    ? Math.max(8, Math.min(viewportWidth - menuWidth - 8, rawLeft))
                                    : rawLeft;
                              ...
                              onMouseLeave={() => setHoveredCategory(null)}
                              className="group relative flex items-center justify-center px-0.5 py-0.5 border-0 bg-transparent hover:scale-105 transition-transform duration-200"
                              style={{
                                background: 'transparent',
                                border: 'none',
                                boxShadow: 'none',
                          ...
                          <div className="flex flex-col items-center py-0.5 space-y-1.5 mt-0">
                            ...
                            <div className="flex flex-col items-center">
                              {currentPage < totalPages && (
                                <span
                                  className="text-[19px] font-black mb-0.5 opacity-90"
                                  style={{ color: currentTheme.text }}
                                >
                                  Next
                                </span>
              ...
              {/* Enhanced Tooltip */}
              <Tooltip
                text={hoveredCategory?.name || ''}
                rect={hoveredCategory?.rect || null}
                visible={!!hoveredCategory}
                position={isVertical ? (panelPosition === 'right' ? 'left' : 'right') : (panelPosition === 'bottom' ? 'top' : 'bottom')}
                theme={currentTheme}
              />
                    ...
                    >
                      <div className="flex items-center justify-between mb-4 pt-2">
                        <div className="flex items-center gap-3 pt-1">
                          <div
                            className={`flex ${selectedCategory === 'az' ? 'h-12 w-12' : 'h-14 w-14'} flex-shrink-0 items-center justify-center rounded-xl overflow-hidden mt-2`}
                            style={{
                          ...
                          >
                            {selectedCategory === 'az' ? (
                              <div className="flex flex-col items-center justify-center leading-none gap-0.5" style={{ color: '#000000', width: '100%', height: '100%', padding: '1px' }}>
                                <span style={{ fontSize: '12px', display: 'block', lineHeight: '1', fontWeight: 'normal', whiteSpace: 'nowrap', overflow: 'visible', textOverflow: 'clip' }}>A to Z</span>
                                <span style={{ fontSize: '12px', display: 'block', lineHeight: '1', fontWeight: 'normal', whiteSpace: 'nowrap', overflow: 'visible', textOverflow: 'clip' }}>List</span>
                              </div>
                            ) : (
                              <Image
                                src={
                                ...
                                alt=""
                                width={selectedCategory === 'position' ? 32 : 38}
                                height={selectedCategory === 'position' ? 32 : 38}
                                className={`${selectedCategory === 'speech' ? '' : 'brightness-0'}`}
                              />
                            )}
                          </div>
                          <div className="flex flex-col pt-1">
                            ...
                            >
                              {selectedCategory === 'az' ? 'A to Z List' : selectedCategory === 'feedback' ? 'Toolbar Feedback' : selectedCategory === 'info' ? 'Features Guide' : (() => {
                                ...
                                return withSpaces.replace(/\n/g, ' ').toLowerCase().split(' ').map((word: string) => {
                        ...
                        <button
                          onMouseEnter={() => {
                            ...
                            if (textToSpeech && !justOpenedRef.current) {
                              speak('Exit Panel', ttsVoiceGender);
                            }
                          }}
                          onMouseLeave={() => setHoveredCategory(null)}
                          onClick={async () => {
                            ...
                            if (textToSpeech) {
                              await speak('Exit Panel', ttsVoiceGender);
                            }
                            setSelectedCategory(null);
                          }}
      ...
      {/* Reset Popup Overlay */}
      {
        showResetConfirm && (
          <div className="accessibility-bar pointer-events-auto fixed inset-0 z-[2147483647] flex items-center justify-center p-3 sm:p-6 animate-in fade-in duration-300">
            ...
            />
            <div
              className="relative rounded-3xl p-5 sm:p-6 shadow-2xl max-w-lg w-full m-4 z-10 animate-scale-up border-[4px] sm:border-[6px] max-h-[90vh] overflow-y-auto"
              style={{
            ...
            >
              <div className="text-center w-full mb-2">
                <div
                  className="w-12 h-12 sm:w-16 sm:h-16 rounded-2xl sm:rounded-3xl flex items-center justify-center mx-auto mb-2 sm:mb-3 group"
                  style={{
              ...
              {!showActiveFeaturesList ? (
                ...
                <div className="flex flex-col gap-3 sm:gap-4 items-center mt-2">
                  {activeProfile && t.profileExplanations && t.profileExplanations[activeProfile] && (
                    <div
                      className="mb-2 p-3 sm:p-4 rounded-xl text-left border-2 animate-in slide-in-from-top-4 duration-500 w-full"
                      style={{
                        backgroundColor: `${currentTheme.text}08`,
                        borderColor: `${currentTheme.border}4D`
                      }}
                    >
                      <p className="text-sm sm:text-base font-black mb-1.5 flex items-center gap-2" style={{ color: currentTheme.text }}>
                        <span className="w-1.5 h-5 rounded-full" style={{ backgroundColor: '#FFD700' }}></span>
                        Active Profile: {ACCESSIBILITY_PROFILES.find(p => p.id === activeProfile)?.name}
                      </p>
                      <p className="text-xs sm:text-sm leading-relaxed font-bold opacity-90" style={{ color: currentTheme.text }}>
                        {t.profileExplanations[activeProfile]}
                      </p>
                    </div>
                  )}
                  <div className="flex flex-wrap gap-2 justify-center mb-0 w-full max-h-[100px] overflow-y-auto icons-scroll-hidden">
      ...
      {
        isOpen && selectedCategory === 'contact' && (
          <ContactPage onClose={() => setSelectedCategory(null)} />
        )
      }
            ...
            >
              <div className="text-center w-full mb-6">
                <div className="flex flex-col items-center justify-center mb-4">
                  ...
                  <h2 className="text-xl sm:text-2xl font-black leading-tight">
                    Text To Speech for the Toolbar Menu titles is on by default. Would you like to?
                  </h2>
                </div>
